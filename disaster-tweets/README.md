...
| **Bi-LSTM + GloVe** | 82.27%   | 사전학습 임베딩 활용           |
| **Bi-LSTM + Word2Vec** | 80.89%   | 도메인 특화 임베딩             |
| **Bi-LSTM + Random** | 79.91%   | 임베딩 baseline                |
...
- Bi-LSTM with GloVe: 82.27% 정확도 (개선된 딥러닝)
- Bi-LSTM with Word2Vec: 80.89% 정확도
- Bi-LSTM with Random: 79.91% 정확도 (baseline)
...
### 발견 4: 임베딩 비교 실험 (딥러닝 개선)

**패턴:** 사전학습 임베딩(GloVe)이 도메인 특화 임베딩(Word2Vec)보다 우수

**임베딩 비교 실험 결과:**
| 임베딩 | 검증 정확도 | 특징 |
|--------|-------------|------|
| **GloVe (사전학습)** | 82.27% | 대규모 코퍼스에서 학습, 일반화 성능 우수 |
| **Word2Vec (도메인)** | 80.89% | 트윗 데이터에서 직접 학습 |
| **Random (baseline)** | 79.91% | 무작위 초기화, 학습만으로 성능 |
...
**해석:**
- 사전학습 임베딩이 소규모 데이터셋에서 효과적
- Bidirectional 구조가 문맥 이해 향상
- 적절한 정규화로 과적합 방지
...
| 모델 | 벡터화/임베딩 | 검증 정확도 | 비고 |
|------|---------------|-------------|------|
| Logistic Regression | TF-IDF | **88.84%** | 최고 성능 |
| Bi-LSTM | GloVe | 82.27% | 딥러닝 최고 |
| Bi-LSTM | Word2Vec | 80.89% | 도메인 특화 |
| Bi-LSTM | Random | 79.91% | baseline |
| Logistic Regression (기본) | TF-IDF | 79.65% | 튜닝 전 |

### 핵심 교훈

1. **단순함의 힘**
   - TF-IDF + 로지스틱 회귀가 여전히 최고 성능
   - 개선된 LSTM(82.27%)도 경쟁력 있는 성능 달성
   - 사전학습 임베딩(GloVe)이 소규모 데이터에서 효과적
